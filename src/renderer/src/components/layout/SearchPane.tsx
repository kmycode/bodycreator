...
import { VerticalAnglePicker, WearOptionPicker, IconGroupPicker, wearIcons, cubeIcons, faceIcons, lineIcons, sleepIcons, armVerticalIcons, legVerticalIcons, armHorizontalIcons, legHorizontalIcons } from "./pickers";
...
function SearchPane(): React.JSX.Element {
  ...
  const handleFaceVerticalChange = useCallback((value: string) => {
    setFaceVertical(value);
  }, [setFaceVertical]);

  const handleFaceHorizontalChange = useCallback((value: string) => {
    setFaceHorizontal(value);
  }, [setFaceHorizontal]);

  const handleArmHorizontalChange = useCallback((value) => {
    setArmHorizontal(value);
  }, [setArmHorizontal]);

  const handleArmVerticalChange = useCallback((value) => {
    setArmVertical(value);
  }, [setArmVertical]);

  const handleElbowChange = useCallback((value) => {
    setElbow(value);
  }, [setElbow]);

  const handleLegHorizontalChange = useCallback((value) => {
    setLegHorizontal(value);
  }, [setLegHorizontal]);

  const handleLegVerticalChange = useCallback((value) => {
    setLegVertical(value);
  }, [setLegVertical]);

  const handleKneeChange = useCallback((value) => {
    setKnee(value);
  }, [setKnee]);

  const handleChestVerticalChange = useCallback((value: string) => {
    setChestVertical(value);
  }, [setChestVertical]);

  const handleChestHorizontalChange = useCallback((value: string) => {
    setChestHorizontal(value);
  }, [setChestHorizontal]);

  const handleSleepChange = useCallback((value: string) => {
    setSleep(value);
  }, [setSleep]);

  const handleWaistVerticalChange = useCallback((value: string) => {
    setWaistVertical(value);
  }, [setWaistVertical]);

  const handleWaistHorizontalChange = useCallback((value: string) => {
    setWaistHorizontal(value);
  }, [setWaistHorizontal]);

  const handleArmWearChange = useCallback((value: string) => {
    setArmWear(value);
  }, [setArmWear]);

  const handleArmWearOptionsChange = useCallback((value: string[]) => {
    setArmWearOptions(value);
  }, [setArmWearOptions]);

  const handleBodyWearChange = useCallback((value: string) => {
    setBodyWear(value);
  }, [setBodyWear]);

  const handleBodyWearOptionsChange = useCallback((value: string[]) => {
    setBodyWearOptions(value);
  }, [setBodyWearOptions]);

  const handleLegWearChange = useCallback((value: string) => {
    setLegWear(value);
  }, [setLegWear]);

  const handleLegWearOptionsChange = useCallback((value: string[]) => {
  ...
  return (
    <div className="searchpane pane">
      ...
      <div className="pane-contents">
        ...
        <h3>腕、肘</h3>
        <div className="searchpane__row">
          <IconGroupPicker icons={armHorizontalIcons} value={armHorizontal} onChange={handleArmHorizontalChange}/>
        </div>
        <div className="searchpane__row">
        ...
        <h3>脚、膝</h3>
        <div className="searchpane__row">
          <IconGroupPicker icons={legHorizontalIcons} value={legHorizontal} onChange={handleLegHorizontalChange}/>
        </div>
        <div className="searchpane__row">
